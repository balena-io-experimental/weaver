...
export const SearchField: FC<SearchFieldProps> = ({ filters }) => {
			...
			sigma
				...
				.forEachNode((key: string, attributes: Attributes): void => {
					if (
						!attributes.hidden &&
						attributes.label &&
						attributes.label.toLowerCase().includes(lcSearch) &&
						!newValues.some((n) => n.label === attributes.label)
					) {
	...
	const onInputChange = (e: ChangeEvent<HTMLInputElement>) => {
		...
		const valueItem = values.find((value) =>
			value.label.includes(searchString),
		);
		if (valueItem) {
