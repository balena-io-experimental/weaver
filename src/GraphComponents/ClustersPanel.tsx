...
import { faObjectGroup, faRectangleXmark, faSquareCheck } from "@fortawesome/free-solid-svg-icons";
import styled from "styled-components";
import { Button, ButtonsWrapper, Txt } from "../SharedComponents";
...
export const ClustersPanel: FC<ClustersPanelProps> = ({ clusters, filters, toggleCluster, setClusters }) => {
  ...
  return (
    ...
    >
      <p>
        <Txt italic muted>Click a file path to show/hide related files and imports.</Txt>
      </p>
      <ButtonsWrapper>
        <Button onClick={() => setClusters(mapValues(keyBy(clusters, "key"), () => true))}>
          ...
          return (
            ...
            >
              ...
              <label htmlFor={`cluster-${cluster.key}`}>
                <span data-type="circle" style={{ background: cluster.color, borderColor: cluster.color }} />{" "}
                <div data-type="node-label">
                  <span>{cluster.clusterLabel}</span>
                  <div data-type="bar" style={{ width: ((100 * nodesCount) / maxNodesPerCluster || 0) + "%" }}>
                    <div
                      data-type="inside-bar"
                      style={{
                        width: ((100 * visibleNodesCount) / nodesCount || 0) + "%"
                      }}
                    />
                  </div>
                </div>
